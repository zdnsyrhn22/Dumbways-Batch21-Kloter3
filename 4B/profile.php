...
if (!isset($_SESSION["login"])) {
    echo "
    <script>
        alert('Anda Belum Login Silahkan login Terlebih Dahhulu!!');
...
$article = $db->query("SELECT *, autho.id AS a_id, autho.name AS a_name, article.image AS a_image, category.id AS c_id, category.name AS c_name, article.id AS art_id  FROM article 
JOIN autho ON article.id_user = autho.id
JOIN category ON article.id_category = category.id
WHERE article.id_user = '$user'");

$profile = $db->find('autho', 'id', $_SESSION['login']['id']);
        ...
        <div class="row">
                        ...
                        <div class="row g-0">
                            ...
                            <div class="col-md-8">
                                <div class="card-body">
                                    ...
                                    <div>
                                        <a class="btn btn-warning" href="update.php?id=<?= $row["art_id"]; ?>">Update</a>
                                        <a class="btn btn-danger" href="delete.php?id=<?= $row["art_id"]; ?>" onclick="return confirm('yakin?');">Delete</a>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="col mt-5 float-end">
                <div class="card" style="width: 18rem;">
                    <div class="card-body">
                        <img src="img/<?= $profile['image']; ?>" class="card-img " style="width: 100px ;">
